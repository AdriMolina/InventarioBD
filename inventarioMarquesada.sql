...
insert into categorias (Nombre,Activo,id_departamento) values('Audifonos',false,2);
insert into categorias (Nombre,Activo,id_departamento) values('Cargadores',true,3);
insert into categorias (Nombre,Activo,id_departamento) values ('protectores',false,4);
...
SELECT id_articulos
			FROM articulos
			ORDER BY id_articulos DESC limit 1;
SELECT id_chip
			FROM chip
			ORDER BY id_chip DESC limit 1;


							select de.nombre
							from departamentos de, colocacion co
							where de.id_colocacion= co.id
							and co.tipo = 'Local';
